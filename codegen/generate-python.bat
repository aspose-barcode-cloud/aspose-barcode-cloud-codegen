...
set targetDir=..\..\aspose-barcode-cloud-python

if exist %tempDir% del /s /q %tempDir% || goto :error

rem java -jar Tools\swagger-codegen-cli.jar config-help -l python
java -jar Tools\swagger-codegen-cli.jar generate -i "%specSource%" -l python -t Templates\python -o %tempDir% -c config-python-go.json || goto :error
rem java -DdebugModels -jar Tools\swagger-codegen-cli.jar generate -i "%specSource%" -l python -t Templates\python -o %tempDir% -c config-python-go.json > debugModels.py.json || goto :error
rem java -DdebugOperations -jar Tools\swagger-codegen-cli.jar generate -i "%specSource%" -l python -t Templates\python -o %tempDir% -c config-python-go.json > debugOperations.py.json || goto :error
